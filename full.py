...
# Open source code by RED-MAFIA! ngotak dikit cok jangan jual di perjual belikan...

## Jangan di jual belikan memek...
## Hargai gue dengan jangan ganti nama author...
## Dan jangan lupa suscribe chenel gue mmk...
## Buat yang mau ngamal ke dana -> 03188214452
## HARUS DANA AJG:V
...
def banner():
 ...
 |/   \__/(_______/|/     \||/     \||/     \||/    )_)
======================================================================
 [\x1b[1;96m+%s] Author   : RIAZKHAN(RK)
 [\x1b[1;96m+%s] Github   : https://github.com/RED-MAFIA
 [\x1b[1;96m+%s] Facebook : MERI ID KO DEKH KE KIA KARO GE
======================================================================\n'''%(N,N,N,N))
...
class login:
	...
	def takon(self):
		...
		if takon in ['y','Y','iya']:
			...
			os.system('xdg-open wa.me/+923188214452');exit()
		else:
			login().__login__()
	def bot(self):
...
class menu:
	...
	def main(self):
		...
		print(' %s║'%(N))
		print(' %s╠═[%s•%s] AGAYE HAN    : %s'%(N,O,N,akss))
		print(' %s╠═[%s•%s] user      id : %s'%(N,O,N,IP))
		print(' %s╠═[%s•%s] ip     adres         : %s'%(N,O,N,waktu))
		print(' %s║'%(N))
		print(' %s╠═[%s0%s] crack FROM (RIAZ) FRIENDS    [ON]'%(N,O,N))
		print(' %s╠═[%s1%s] crack FROM (RIAZ) PUBLIC      [ON]'%(N,O,N))
		print(' %s╠═[%s2%s] crack FROM PUB/FRIND (RK)   [ON]'%(N,O,N))
		print(' %s╠═[%s3%s] crack FROM (RIAZ)postingan   [ON]'%(N,O,N))
		print(' %s╠═[%s4%s] crack FROM likes post          [ON]'%(N,O,N))
